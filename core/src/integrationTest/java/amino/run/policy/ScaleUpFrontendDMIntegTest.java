...
public class ScaleUpFrontendDMIntegTest {
    ...
    private void runTest(MicroServiceSpec spec) throws Exception {
        ...
        try {
            ...
            // Due to scaling up, it may throw ServerOverLoadException or
            // KernelObjectMigratingException
            // or ScaleUpException which has been caught here. So, exceptions other than these will
            // be
            // thrown.
                ...
                try {
                    ...
                    if (expectedExceptions.contains(e.getCause().getCause().getClass())) {
                        ...
                        if (i == 0) {
                            ...
                            for (int k = 0; k < i; k++) {
                                int replica_count =
                                        LoadBalancedFrontendPolicy.DEFAULT_REPLICA_COUNT;
                                // There is a possibility that set happens in one server and get
                                // from a
