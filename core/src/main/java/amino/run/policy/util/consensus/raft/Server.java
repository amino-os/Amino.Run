...
import static amino.run.policy.util.consensus.raft.PersistentState.INVALID_INDEX;
...
/**
 * Created by quinton on 1/31/18. Implementation of the RAFT consensus algorithm, as per
 * https://www.usenix.org/system/files/conference/atc14/atc14-paper-ongaro.pdf Not fully tested or
 * production-ready yet. Please read the above paper, or at least p308 before trying to understand
 * or working on this code. I considered using an existing Java Server library, but no obviously
 * good candidates could be found, so decided to just try to implement Server as per the paper. May
 * be replaced in future by alternative RAFT implementation.
 */
public class Server
        implements RemoteRaftServer { // This outer class contains everything common to leaders,
     ...
     * Constructor
     *
     * <p>Note that the constructor does not start the RAFT algorithm. To do that, call addServer()
     * for each server, and then call start()
     */
     ...
     * @param entries log entries to store (empty for heartbeat; may send more than one for
     *     efficiency)
    ...
    public int appendEntries(
        ...
        if (term < pState.getCurrentTerm()) {
            throw new InvalidTermException(
                    "Server: Attempt to append entries from prior leader term "
                            + term
                            + ", current term "
    ...
    public int requestVote(int term, UUID candidate, int lastLogIndex, int lastLogTerm)
        ...
        if (!candidate.equals(
                pState.myServerID)) { // We sometimes vote for ourselves, to that's not considered a
            // request from a remote server.
            /** All servers convert to followers if their current term is behind (§5.1). */
    ...
    /**
     * Apply log entries that have been committed but not yet applied. • If commitIndex >
     * lastApplied: increment lastApplied, apply log[lastApplied] to state machine (§5.3)
     */
    void applyCommitted() {
        int lastApplied;
        while (vState.getCommitIndex() > (lastApplied = vState.getLastApplied())) {
            ...
            try {
                ...
                logger.warning(
                        String.format(
                                "Operation %s generated exception %s on replica.  "
            ...
            // run method calls in the same sequence).  So in the absence of external side effects
            // that
    ...
    public Object applyToStateMachine(Object operation) throws java.lang.Exception {
        ...
        if (vState.getState() == State.LEADER) {
            ...
            throw new LeaderException(
                    String.format("Current Leader is %s", vState.getCurrentLeader()),
                    getCurrentLeader());
        }
    }
    ...
    class Leader {
        ...
        void start() {
            ...
            appendEntriesThreadPool =
                    (ThreadPoolExecutor)
                            Executors.newFixedThreadPool(vState.otherServers.size() * 2 + 1);

            leaderHeartbeatSendTimer =
                    new ResettableTimer(
                            new TimerTask() {
                                public void run() {
                                    sendHeartbeats();
             ...
             * entries starting at nextIndex • If successful: update nextIndex and matchIndex for
             * follower (§5.3) • If AppendEntries fails because of log inconsistency: decrement
             * nextIndex and retry (§5.3) • If there exists an N such that N > commitIndex, a
            ...
            while (!success && vState.getState() == State.LEADER) {
                ...
                try {
                    ...
                    this.nextIndex.put(
                        ...
                        InvalidLogIndex
                                e) { // The remote server doesn't have that log entry at all.
        ...
        void sendHeartbeats() { // We might want to keep a separate timer for each other server.
            // For now, we use one timer.
            if (vState.getState() == State.LEADER) {
                final Iterator<UUID> i = vState.otherServers.keySet().iterator();
                while (i.hasNext()) {
                    final UUID server = i.next();
                    appendEntriesThreadPool.execute(
                            new Runnable() {
                                @Override
                                public void run() {
                                    ...
                                    try {
                                        ...
                                        logger.warning(
                                                String.format(
                                                        "%s: Applying operatation remotely on %s generated exception %s.  This can probably be ignored.",
        ...
        boolean shouldBeCommitted(int logIndex) {
            ...
            for (UUID otherServerID : vState.otherServers.keySet()) {
                ...
                if (match >= logIndex) {
                    if (++matches
                            >= majorityQuorumSize()
                                    - 1) { // -1 because the leader implicitly matches
                        return true;
                    }
                }
            }
         ...
         * log[N].term = currentTerm, then set commitIndex = N (see 5.3, 5.3)
         */
        ...
        /**
         * Applies an operation to the state machine, after successfully appending it to the logs of
         * a majority quorum of replicas.
         *
         * @param operation to apply, i.e. a method invocation
         * @return result of the method invocation
         * @throws java.lang.Exception either an exception thrown by the method invocation when
         *     applied locally, or a RAFTException indicating why the operation could not be
         *     applied.
         */
        ...
        // method in different speed. We further assume that the lastLogIndex observed by thread A
        // is
        ...
        public Object applyToStateMachine(Object operation) throws java.lang.Exception {
            ...
            final Semaphore replicationCounter =
                    new Semaphore(0); // Initially we have zero replicas
            final Iterator<UUID> i = vState.otherServers.keySet().iterator();
            while (i.hasNext()) {
                        ...
                        new Runnable() {
                            ...
                            public void run() {
                                if (otherServerNextIndex == null
                                    ...
                                    logger.warning(
                                            String.format(
                                                    "%s: Whooah! logIndex %d is not greater than otherServerNextIndex %s",
                                                    pState.myServerID,
                                                    logIndex,
                                                    otherServerNextIndex));
                                }
            ...
            if (vState.getLastApplied() >= logIndex) {
                ...
                logger.severe(
                        String.format(
                                "Whooah! Internal error: logIndex %d was committed by a majority quorum, but not applied locally.",
                                logIndex));
            }
            return returnVal;
        }
        ...
        /**
         * Apply log entries that have been committed but not yet applied. • If commitIndex >
         * lastApplied: increment lastApplied, apply log[lastApplied] to state machine (§5.3). On
         * the master, return the result of the last applied operation, for return to the client.
         */
    ...
    int fullQuorumSize() {
        return vState.otherServers.size()
                + 1; // NOTE: This only makes sense if otherServers contains all the other servers
        // at this time.
    }

    RemoteRaftServer getServer(UUID serverId) {
        return vState.otherServers.get(
                serverId); // TODO: Handle not found - which would indicate a code bug.
    }
    ...
    class Follower {
        ...
        void start() {
            /**
             * Followers (§5.2) - Respond to RPCs from candidates and leaders (we already handle
             * appendEntries() and requestVote() ) - If election timeout elapses without receiving
            ...
            if (null == leaderHeartbeatReceiveTimer) {
                ...
                leaderHeartbeatReceiveTimer =
                        new ResettableTimer(
                                new TimerTask() {
                                    public void run() {
                                        /**
                                         * If we don't receive a heartbeat from the leader, start an
                                         * election.
                                         */
    ...
    class Candidate {
        ...
        void start() {
            /**
             * Candidates (§5.2): - On conversion to candidate, start election: - Increment
             * currentTerm - Vote for self - Reset election timer - Send RequestVote RPCs to all
             * other servers - If votes received from majority of servers: become leader - If
        ...
        /**
         * Send vote requests to all other servers. Returns once all vote requests have been queued
