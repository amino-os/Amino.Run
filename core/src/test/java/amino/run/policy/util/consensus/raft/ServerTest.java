...
public class ServerTest {
    ...
    public void successfulLeaderElection() throws java.lang.InterruptedException {
        ...
        Thread.sleep(100); // Wait a while and make sure that we still have exactly one leader,
    ...
    public void testPersistence() throws java.lang.Exception {
        ...
        FileChannel fc = new RandomAccessFile(new File(persistFile), "r").getChannel();
        buffer = fc.map(FileChannel.MapMode.READ_ONLY, 0, fc.size());

        // Verifying currentTerm is the same
        assertEquals(raftServer[0].pState.getCurrentTerm(), buffer.getInt());

        // Extracting votedFor from the persistent storage
        bytes = new byte[buffer.getInt()];
        buffer.get(bytes);
        String votedFor = new String(bytes);
        // Verifying votedFor is same
        assertEquals(raftServer[0].pState.getVotedFor(), UUID.fromString(votedFor));

        // Extracting myServerID from the persistent storage
        bytes = new byte[buffer.getInt()];
        buffer.get(bytes);
        String myServerID = new String(bytes);
        // Verifying myServerID is same
        assertEquals(raftServer[0].pState.myServerID, UUID.fromString(myServerID));

        while (buffer.hasRemaining() && indx < raftServer[0].pState.log().size()) {
