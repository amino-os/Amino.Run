...
java -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=22222 -Djava.util.logging.config.file=${conf_prop} -cp ${cp_app}:${cp_sapphire}:${cp_harmony} sapphire.kernel.server.KernelServerImpl  localhost ${port_kernel} localhost ${port_oms}  
