...
public class UpdaterTask extends AsyncTask<String, Void, Void> {
	...
	private PlayStoreMetadataFetcherResult getAppData() throws IOException, JSONException {
		...
		try {
			...
			// todo: get versionCheckURL properly: URLConnection conn = new URL(BuildConfig.VERSION_CHECK_URL).openConnection();
			String versionCheckURL = "";
			URLConnection conn = new URL(versionCheckURL).openConnection();
