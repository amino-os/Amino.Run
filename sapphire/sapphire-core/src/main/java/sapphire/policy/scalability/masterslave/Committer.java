...
import static sapphire.policy.scalability.masterslave.MethodInvocationResponse.ReturnCode.SUCCESS;


/**
 * Committer applies method invocation requests onto App objects. It synchronizes all
 * reads and writes to the App object.
 *
 * @author terryz
 */
public class Committer implements Closeable {
        ...
        if (executor != null) {
            ...
            try {
                if (! executor.awaitTermination(config.getShutdownGracePeriodInMillis(), TimeUnit.MILLISECONDS)) {
                    // shutdown time out
                    logger.log(Level.SEVERE,"Committer shut down time out after {0} milliseconds", config.getShutdownGracePeriodInMillis());
                }
            } catch (InterruptedException e) {
                logger.log(Level.SEVERE, String.format("got exception during Committer shut down: %s", e), e);
            }
        }
    }
            ...
            public Object call() throws Exception {
                synchronized (appObject) {
                    try {
                        ...
                        logger.log(Level.FINER, "result for request {0}: {1}", new Object[]{request, result});
                        return result;
                    } catch (Exception e) {
                        logger.log(Level.SEVERE, String.format("failed to invoke write request %s on app object: " +
                                "%s", request, e), e);
                        throw e;
                    }
                }
     ...
     *
     * Entry indices have to increase monotonically
     *
     * @param request
     * @param entryIndex
     * @return
    ...
    public Future<Object> applyWriteAsync(final MethodInvocationRequest request, final long entryIndex) {
        ...
        return executor.submit(new Callable<Object>() {
            ...
            if (entryIndex <= getIndexOfLargestCommittedEntry()) {
                String msg = String.format("forbidden to commit old entry %s " +
                                "against the current largest committed entry %s", entryIndex,
                        getIndexOfLargestCommittedEntry());
     ...
     *
     * @param object the new value of the App object. After this method call, the App object
     *               managed by this committer will be replaced with this object.
    ...
    private MethodInvocationResponse createResponse(MethodInvocationRequest request, Future future) {
        ...
        if (future == null) {
            response = new MethodInvocationResponse(FAILURE, new Exception("executor not initialized"));
        } else {
            try {
                Object result = future.get();
                response = new MethodInvocationResponse(SUCCESS, result);
                logger.log(Level.FINER, "response for request {0}: {1}", new Object[]{request, response});
                ...
                // This is likely caused by Sapphire errors. We treat it as an application error for now.
                // The problem is: the other replica may not run into this exception. This exception
                // may cause two replicas out of sync.
                SapphireException ex = new SapphireException("Method invocation on app object was interrupted", e);
