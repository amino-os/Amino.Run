...
public class ConsensusRSMPolicyTest implements Serializable{
    ...
    public static class Server_Stub extends ConsensusRSMPolicy.ServerPolicy implements KernelObjectStub {
    ...
    public void setUp() throws Exception {
        ...
        this.server1 = (ConsensusRSMPolicy.ServerPolicy) spy(KernelObjectFactory.create(Server_Stub.class.getName()));
        ...
        final KernelObjectStub spiedReplicaServerStub1 = spy(KernelObjectFactory.create(Server_Stub.class.getName()));
        final KernelObjectStub spiedReplicaServerStub2 = spy(KernelObjectFactory.create(Server_Stub.class.getName()));
        ...
        ArrayList<SapphirePolicy.SapphireServerPolicy> servers = this.client.getGroup().getServers();
    ...
    public void requestVote() throws Exception {
        ...
        verify(this.server1, times(1)).requestVote(anyInt(),(UUID) anyObject(),anyInt(),anyInt());
    }

    @Test
    public void appendEntries() throws Exception {
        this.server1.appendEntries(anyInt(),(UUID) anyObject(),anyInt(),anyInt(),anyList(),anyInt());
        verify(this.server1, times(1)).appendEntries(anyInt(),(UUID) anyObject(),anyInt(),anyInt(),anyList(),anyInt());
    }
    ...
    /**
     * Try onRPC on the first server, if this is not the leader throw LeaderException.
     * Retry rpc on the next server, if leader not elected throw RemoteException.
     */
    @Test
    public void onRPCWithoutLeader() throws Exception {
        ...
        doThrow(new LeaderException("leaderException", null)).when(this.server1).onRPC(method, params);
    ...
    /**
     * Try onRPC on the first server, if this is not the leader throw LeaderException.
     * Retry rpc on the leader server.
     */
    @Test
    public void onRPCWithLeader() throws Exception {
        ...
        doThrow(new LeaderException("leaderException", this.server2)).when(this.server1).onRPC(method, params);
        this.client.onRPC(method, params);
    }

    /**
     * If onRPC fails on the leader, get servers from group and find a responding server.
     * Try with next server until you find a responding server.
     */
    @Test
    ...
    /**
     * If onRPC fails on the leader, get servers from group and find a responding server.
     * If leader still not elected, throw RemoteException.
     */
    @Test
    public void onRPCToFollowerWithoutLeader() throws Exception {
        ...
        doThrow(RemoteException.class).when(this.server1).onRPC(method, params);
        doThrow(new LeaderException("LeaderException", null)).when(this.server2).onRPC(method, params);
        thrown.expect(RemoteException.class);
        thrown.expectMessage("Raft leader is not elected");
        this.client.onRPC(method, params);
    }
    ...
    /**
     * If onRPC fails on the leader, get servers from group and find a responding server.
     * Store this server as reachable and use it for future rpc.
     */
    @Test
    public void onRPCToFollowerWithLeader() throws Exception {
        ...
        doThrow(RemoteException.class).when(this.server1).onRPC(method, params);
        doThrow(new LeaderException("LeaderException", this.server3)).when(this.server2).onRPC(method, params);
        this.client.onRPC(method, params);
    }
}