...
public class LoadBalancedMasterSlavePolicyTest {
    ...
    public void testRenewLockSucceeded() throws Exception {
        ...
        Lock masterLock = new Lock(clientId, logIndex, lockTimeoutInMillis);
        setMasterLock(group, masterLock);
        Assert.assertTrue("renew should succeed", group.renewLock(clientId, logIndex));
    }
    ...
    public void testRenewLockFailedWithExpriedLock() throws Exception {
        ...
        Thread.sleep(100L);
        boolean result = group.renewLock(clientId, logIndex);
        Assert.assertFalse("renew should fail because lock has expired", group.renewLock(clientId, logIndex));
    }
    ...
    public void testObtainLockSucceeded() throws Exception {
        ...
        Thread.sleep(100L);
        Assert.assertTrue("obtain lock should succeed", group.obtainLock(clientId, logIndex));
    }
    ...
    public void testObtainLockFailedWithUnexpiredLock() throws Exception {
        ...
        Assert.assertFalse("obtain lock should fail", group.obtainLock("AnotherClient", logIndex));
    }
